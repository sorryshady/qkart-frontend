          ...
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              ...
              {addresses.all.length > 0 ? (
                addresses.all.map((adder, idx) => {
           
                  return (
                    <div key={adder._id} style={{ marginTop: "1rem" }}  >
                      <Box 
                        sx={{
                          border: 1,
                          borderRadius: 1,
                      ...
                      >
                        <Grid container className={`address-item${addresses.selected=== adder._id ?  " selected":  " not-selected"}`} onClick={() => {(addresses.selected = adder._id);
                        console.log('Address selected', adder._id);
                        setAddState(btnState => !btnState);}}>
                          <Grid item md={10}>
                            <Typography my="1rem">{adder.address}</Typography>
                          </Grid>
                          <Grid item md={2}>
                            <Button
